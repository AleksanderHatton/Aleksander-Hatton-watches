...
- Added parallel direct uploads with visible progress feedback for catalogue and valuation photos.
...
- Stopped dealer notes from saving on every keystroke; they now use explicit Save Notes controls.
...
- Removed duplicate legacy root source files so the repository has one clear active code path.
